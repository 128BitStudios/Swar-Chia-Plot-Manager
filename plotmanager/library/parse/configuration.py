...
def _get_progress_settings(config):
    ...
    _check_parameters(parameter=progress_setting,
                      expected_parameters=expected_parameters, parameter_type='progress')
...
def _get_manager_settings(config):
    if 'manager' not in config:
        raise InvalidYAMLConfigException(
            'Failed to find the log parameter in the YAML.')
    manager = config['manager']
    expected_parameters = ['check_interval', 'log_level']
    _check_parameters(
        parameter=manager, expected_parameters=expected_parameters, parameter_type='manager')
...
def _get_log_settings(config):
    ...
    _check_parameters(
        parameter=log, expected_parameters=expected_parameters, parameter_type='log')
...
def _get_notifications_settings(config):
    ...
    expected_parameters = ['notify_discord', 'discord_webhook_url', 'notify_sound', 'song', 'notify_pushover',
                           'pushover_user_key', 'pushover_api_key', 'notify_broadcast_interval', 'broadcast_webhook_url']
    _check_parameters(parameter=notifications,
                      expected_parameters=expected_parameters, parameter_type='notification')
...
def _get_view_settings(config):
    if 'view' not in config:
        raise InvalidYAMLConfigException(
            'Failed to find view parameter in the YAML.')
    view = config['view']
    ...
    _check_parameters(
        parameter=view, expected_parameters=expected_parameters, parameter_type='view')
    return view
